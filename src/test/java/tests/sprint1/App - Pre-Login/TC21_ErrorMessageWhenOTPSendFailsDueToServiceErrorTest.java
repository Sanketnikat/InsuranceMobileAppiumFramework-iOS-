...
public class TC21_ErrorMessageWhenOTPSendFailsDueToServiceErrorTest extends BaseTest {

    @Test(
            description = "SC_01_TC_021 - Verify appropriate error message is displayed when OTP send fails due to service error",
            groups = {"sprint1", "sprint2", "pre-login", "negative"}
    )
    public void TC21_errorMessageWhenOtpSendFailsDueToServiceError() {
        ...
        // Step 7 - Wait for service response
        System.out.println("Step 7 - Waiting for OTP service response");
        ...
        try {
            ...
            Assert.fail(
                    "Interrupted while waiting for OTP service response."
            );
        }
        ...
        // Step 9 - Check for service error message
        boolean serviceErrorDisplayed =
                pageSource.contains("service error")
                        || pageSource.contains("Service error")
                        || pageSource.contains("service unavailable")
                        || pageSource.contains("Service unavailable")
                        || pageSource.contains("something went wrong")
                        || pageSource.contains("Something went wrong")
                        || pageSource.contains("unable to")
                        || pageSource.contains("Unable to")
        ...
        Assert.assertTrue(
                serviceErrorDisplayed,
                "Appropriate error message for OTP service failure was not displayed."
        );

        System.out.println(
                "Appropriate OTP service error message is displayed."
        );

        System.out.println(
                "SC_01_TC_021 PASSED - OTP service error message displayed."
        );
    }
}