...
public class TS_01_TC_002_ProposalDraftLoadingSkeletonTest extends BaseTest {
    ...
    // Slow BrowserStack network profile so the draft fetch is slow enough to observe the skeleton.
    private static final String SLOW_NETWORK_PROFILE = "3g-lossy-network";

    @Override
    public void setUp(ITestResult result) {
        System.setProperty("networkProfile", SLOW_NETWORK_PROFILE);
        super.setUp(result);
    }
    ...
    public void tearDown(ITestResult result) {
        try {
            ...
            System.clearProperty("networkProfile");
        }
    }

    @Test(
            description = "TS_01_TC_002 - Verify that a loading skeleton is displayed while the proposal draft is being fetched",
            groups = {"sprint5", "BD2M-609", "proposal", "positive"}
    )
    public void proposalDraftLoadingSkeletonTest() {
        ...
        // Step 2: Navigate to the Summary & Consent screen
        clickText("Save & Next");

        // Step 3: Observe the screen while the draft API is in progress
        boolean skeletonObserved = observeLoadingSkeletonUntilDraftLoaded();

        Assert.assertTrue(
                skeletonObserved,
                "Loading skeleton was not displayed while the proposal draft was being fetched."
        );
        ...
        System.out.println();
        System.out.println("=================================================");
        System.out.println("TS_01_TC_002 EXECUTION COMPLETED SUCCESSFULLY");
        System.out.println("Loading skeleton observed while proposal draft was being fetched.");
        System.out.println("=================================================");
    }

    /**
     * Polls the Summary & Consent screen until the proposal draft finishes loading,
     * returning true if a loading skeleton/indicator was seen at least once beforehand.
     */
    private boolean observeLoadingSkeletonUntilDraftLoaded() {
        Instant deadline = Instant.now().plusSeconds(30);
        boolean skeletonSeen = false;

        while (Instant.now().isBefore(deadline)) {
            if (isProposalFormDisplayed()) {
                break;
            }

            if (isLoadingSkeletonDisplayed()) {
                skeletonSeen = true;
                System.out.println("Loading skeleton detected in place of the review card.");
            }
